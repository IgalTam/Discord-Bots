...
from queueing import VoiceError, VoiceState, AudioItem
from botaudioutils import YTDLSource, YTDLError

class BadAlexa(commands.Cog):
    ...
    async def on_ready(self):
        print("BadAlexa Cog loaded.")
    
    # @commands.hybrid_command(name="tbd", with_app_command=True, description="tbd")
    ...
    # @commands.hybrid_command(name="play_from_url", with_app_command=True, description="<URL> adds the song from URL to queue and plays if it is the first queue entry")
    # async def _play_from_url(self, ctx: commands.Context, url: str):
    ...
    #     async with ctx.typing():
    #         try:
    #             source = await YTDLSource.source_from_url(ctx=ctx, url=url, loop=self.bot.loop)
    #         except YTDLError as e:
    #             await ctx.send('An error occurred while processing this request: {}'.format(str(e)))
    #         else:
    #             song = AudioItem(source)
    ...
    async def _clear_queue(self, ctx: commands.context):
        """clears the queue, leaving any connected voice channel"""
    ...
    # @commands.hybrid_command(name="add_from_url", with_app_command=True, description="<url> adds song from URL to queue")
    # async def _add_from_url(self, ctx: commands.Context, url: str):
    #     """Adds the song from a URL to the queue (doesn't start playing)"""
        
    #     async with ctx.typing():
    #         try:
    #             source = await YTDLSource.create_source_from_url(url=url, loop=self.bot.loop)
    #         except YTDLError as e:
    #             await ctx.send('An error occurred while processing this request: {}'.format(str(e)))
    #         else:
    #             song = AudioItem(source)
...
async def setup(bot: commands.Bot):
    await bot.add_cog(BadAlexa(bot))